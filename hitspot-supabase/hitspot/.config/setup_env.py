...
def populate_file(path, file_name, supabase_url, supabase_anon_key, supabase_service_role, google_maps_key):
	with open(f"{path}/.env.{file_name}", 'w') as f:
...
def create_env_file():
	...
	if os.path.exists(f".env.{file_name}"):
	...
	if populate.lower() == 'y':
		supabase_path = input(bcolors.OKCYAN + "Enter the path to the Supabase project\n(default: .): " + bcolors.ENDC)
...
def choose_env_file():
	...
	try:
		...
		if choice > 0 and choice <= len(env_files):
...
def check_environment():
	print(bcolors.HEADER + "Checking current environment." + bcolors.ENDC)
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description=bcolors.HEADER + 'Manage environment configuration files.' + bcolors.ENDC)
	parser.add_argument('action', choices=['create', 'choose', 'check', 'remove'], help=bcolors.UNDERLINE + 'Action to perform: check current environment, create or choose an env file.' + bcolors.ENDC)

	args = parser.parse_args()

	workdir = input(bcolors.OKCYAN + "Enter the path to root of your project\n(default: ../): " + bcolors.ENDC)
	if not workdir:
		workdir = "../"
	try:
		os.chdir(workdir)
